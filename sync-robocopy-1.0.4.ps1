...
while ($global:IsRunning) {
    ...
    # ============================================================================
    $RobocopyArgs = @(
        "`"$SourceDir`"", "`"$DestDir`"", "/MOVE", "/E", $CopyMode, "/DCOPY:DAT",
    ...
    # ============================================================================
    if ($ExitCode -ge 8) {
        # Fallback 1: Auditoría
        if ($CopyMode -eq "/COPYALL" -and ($Output -match "Manage Auditing" -or $Output -match "Auditoría" -or $Output -match "Security may not be copied")) {
            Write-Log "Permisos de Administrador insuficientes para copiar Auditoría (U). Cambiando automáticamente a /COPY:DATSO para los próximos intentos..." "WARN"
            $CopyMode = "/COPY:DATSO"
            $WaitTime = 2 # Reintento casi inmediato
            continue
        }

        # Fallback 2: Permisos NTFS y Propietario
        if ($CopyMode -eq "/COPY:DATSO" -and ($Output -match "Acceso denegado" -or $Output -match "Access is denied" -or $Output -match "ERROR 5 ")) {
            Write-Log "Permisos insuficientes en el destino para escribir ACLs o Propietario (S,O). Cambiando automáticamente a copia estándar /COPY:DAT..." "WARN"
            $CopyMode = "/COPY:DAT"
            $WaitTime = 2 # Reintento casi inmediato
            continue
        }
